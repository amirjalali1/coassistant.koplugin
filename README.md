...
- **X-Ray that keeps up** → optional **Automatic X-Ray** (per-book or global: background auto-update as you read, auto-create early in a book), **prepared versions** (pre-build spoiler-safe X-Ray versions for the whole book, snapped to chapter ends, silently swapped in as you read), and **version history** (browse, restore, or delete earlier X-Ray snapshots, with a keep-count setting)
...
**Background features:** this same consent covers background runs you opt into separately — [Automatic X-Ray](#x-ray-auto-update) and [X-Ray version preparation](#x-ray-version-ladder) extract text and spend tokens **without a per-request confirmation**. They re-check consent at fire time (revoking it stops them), their size is bounded by their own guards (the progress-gap window; incremental preparation steps), and the interactive truncation/size warnings don't apply: an oversized background extraction aborts the run instead of asking.
...
- **Options menu** (☰): info (model, progress, date, fiction/non-fiction/academic type, prepared-version provenance when prepared versions exist), **All versions (N)…** (main X-Ray only, when archived versions exist, see [X-Ray version history](#x-ray-version-history) below), delete, close
...
- From the **X-Ray popup** (open book or file browser) or the X-Ray browser's **☰ menu**, tap **"All versions (N)…"** (main X-Ray only, sections have no history). The list holds both archived *previous* versions and any [prepared](#x-ray-version-ladder) versions — which can sit *ahead* of your reading position, hence "All".
...
- It only **updates** an existing incremental X-Ray or **creates** a first one early in a book (where allowed, per above), and never touches complete, AI-knowledge, legacy, or section X-Rays. It never prepares [versions ahead](#x-ray-version-ladder) — that is always an explicit, cost-confirmed action.
...
**Preparing X-Ray versions ahead (manual, runs in the background):** from the X-Ray popup's Versions group, **"Prepare versions ahead…"** generates a full set of prefix versions of the book's X-Ray — roughly one every 10% of a normal-length book (spacing adapts to book length: wider on short books so a novella isn't sliced into tiny calls, narrower on very long books so a single version never covers an oversized slice; versions snap to chapter ends when the book has a usable table of contents, so they read as "up to the end of a chapter"). The confirmation dialog shows the planned count and spacing, and **"Adjust spacing for this run…"** lets you override it for that one run — e.g. denser versions for dense material. After that one dialog, the versions are built in the background one incremental step at a time — the book is read once in total, plus a fixed per-version overhead, so somewhat more than one full X-Ray run — while you keep reading (the book must stay open; you can cancel anytime and resume later from where it stopped). As you read past a version's position, it is silently swapped in as your live X-Ray, so the X-Ray follows your reading position with zero waiting; all versions stay browsable under **"All versions"**.

Prepared versions serve two purposes, and both are first-class: **spoiler-safe reading** (every position gets an X-Ray that never runs ahead of you), and a **checkpointed full build** — completing a big book's X-Ray in bounded increments instead of one oversized request (a failure keeps the finished versions; resume continues from the highest). If you prepared them for the second reason and don't want spoiler gating, tap **"Switch to complete version (100%) — instant"** in the popup's Versions group: it installs the finished 100% version as your live X-Ray for free (no merging is ever needed — each prepared version already *contains* everything below it), then offers to delete the other prepared versions (keeping them is fine too; they stay browsable). The switch is not a one-way door: while the prepared versions are kept, **"Switch back to your position (NN%) — instant"** re-installs the version at or below where you're reading, and position-tracking (promotion as you read) resumes — both switches are free and reversible. Useful before a re-read, for book clubs, or to give a book spoiler-safe X-Rays for its whole length up front.
...
- **Maximum Progress Gap (%)**: Gaps bigger than this never auto-update — run the update manually from the X-Ray popup, which shows the size first. Also the upper edge of the auto-create window, and the cap on automatic prepared-version swaps after a position jump.
- **Cooldown (minutes)**: Minimum time between background attempts (0 = no cooldown).
- **Notify on Background Activity**: Show a brief notification when a background X-Ray update or create starts and completes, and when a prepared version is silently swapped in. Off = fully silent.
- **Prepared Versions at Chapter Ends**: When [preparing X-Ray versions ahead](#x-ray-version-ladder), place each version at the nearest chapter end (within a few percent) so versions read as "up to the end of a chapter"; the versions list shows the chapter names. Needs a table of contents. (default: ON)
