...
- It only **updates** an existing incremental X-Ray or **creates** a first one early in a book (where allowed, per above), and never touches complete, AI-knowledge, legacy, or section X-Rays. It never builds [checkpoints ahead](#x-ray-version-ladder) — that is always an explicit, cost-confirmed action.
