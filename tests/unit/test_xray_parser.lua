...
TestRunner:suite("rename fold — reverse-alias bridge (Pamela case, 2026-08-14)")
TestRunner:test("new entry bridging an old primary name folds as a rename", function()
    local old = XrayParser.parse('{"characters":[{"name":"Pamela Lovelace","role":"Supporting","description":"old text","connections":["Saladin Chamcha (husband)"],"background":[{"source":"Vol 1","text":"kept"}]}]}')
    local new = XrayParser.parse('{"characters":[{"name":"Pamela Chamcha","aliases":["Pamela Lovelace"],"description":"new text","connections":["Jumpy Joshi (lover)"]}]}')
    local merged = XrayParser.merge(old, new)
    TestRunner:eq(#merged.characters, 1, "one entity, not a split")
    TestRunner:eq(merged.characters[1].name, "Pamela Chamcha", "new name is primary")
    TestRunner:eq(merged.characters[1].description, "new text")
    local aliases = table.concat(merged.characters[1].aliases or {}, "|")
    TestRunner:ok(aliases:find("Pamela Lovelace", 1, true) ~= nil, "old name rides in aliases")
    local conns = table.concat(merged.characters[1].connections or {}, "|")
    TestRunner:ok(conns:find("Saladin Chamcha", 1, true) ~= nil, "old connections union in")
    TestRunner:ok(conns:find("Jumpy Joshi", 1, true) ~= nil, "new connections lead")
...
TestRunner:test("fold union: bare connection yields to the annotated variant", function()
    local old = XrayParser.parse('{"characters":[{"name":"Pamela Lovelace","connections":["Saladin Chamcha (husband)"]}]}')
    local new = XrayParser.parse('{"characters":[{"name":"Pamela Chamcha","aliases":["Pamela Lovelace"],"connections":["Saladin Chamcha","Jumpy Joshi"]}]}')
    local merged = XrayParser.merge(old, new)
    local conns = merged.characters[1].connections
    TestRunner:eq(#conns, 2, "bare duplicate dropped")
    local joined = table.concat(conns, "|")
    TestRunner:ok(joined:find("Saladin Chamcha (husband)", 1, true) ~= nil, "annotated variant survives")
    TestRunner:ok(joined:find("Jumpy Joshi", 1, true) ~= nil)
end)
TestRunner:test("single-word bridge does NOT fold (short-form collision guard)", function()
    local old = XrayParser.parse('{"characters":[{"name":"Khalid","description":"a porter"}]}')
    local new = XrayParser.parse('{"characters":[{"name":"Khalid al-Walid","aliases":["Khalid"],"description":"a general"}]}')
    local merged = XrayParser.merge(old, new)
...
TestRunner:test("ambiguous bridge (two old entries matched) does NOT fold", function()
    local old = XrayParser.parse('{"characters":[{"name":"Pamela Lovelace"},{"name":"Pamela Smith"}]}')
    local new = XrayParser.parse('{"characters":[{"name":"Pamela Chamcha","aliases":["Pamela Lovelace","Pamela Smith"]}]}')
    local merged = XrayParser.merge(old, new)
    TestRunner:eq(#merged.characters, 3, "appended on ambiguity")
end)
TestRunner:test("never-merge pair blocks the fold", function()
    local old = XrayParser.parse('{"characters":[{"name":"Pamela Lovelace"}]}')
    local new = XrayParser.parse('{"characters":[{"name":"Pamela Chamcha","aliases":["Pamela Lovelace"]}]}')
    local merged = XrayParser.merge(old, new,
        { never_pairs = { { "Pamela Chamcha", "Pamela Lovelace" } } })
...
TestRunner:test("forward alias match still wins over the rename fold", function()
    ...
    local old = XrayParser.parse('{"characters":[{"name":"Pamela Chamcha","aliases":["Pamela Lovelace"],"description":"kept identity"}]}')
    local new = XrayParser.parse('{"characters":[{"name":"Pamela Lovelace","description":"rewrite"}]}')
    local merged = XrayParser.merge(old, new)
    TestRunner:eq(#merged.characters, 1)
    TestRunner:eq(merged.characters[1].name, "Pamela Chamcha", "primary name preserved")
end)
